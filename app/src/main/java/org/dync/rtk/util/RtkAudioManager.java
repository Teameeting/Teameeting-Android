...
/**
 * RTCAudioManager manages all audio related parts of the AppRTC demo. 说明：
 * Audio管理器
 * 
...
public class RtkAudioManager {
	...
	private RtkProximitySensor proximitySensor = null;
	...
	private void onProximitySensorChangedState() {
		...
		if (audioDevices.size() == 2 && audioDevices.contains(AudioDevice.EARPIECE)
			...
			if (proximitySensor.sensorReportsNearState()) {
				// Sensor reports that a "handset is being held up to a person's
				// ear",
				// or "something is covering the light sensor".
				setAudioDevice(AudioDevice.EARPIECE);
			} else {
				// Sensor reports that a "handset is removed from a person's
				// ear", or
	...
	/** Construction */
	public static RtkAudioManager create(Context context, Runnable deviceStateChangeListener) {
		return new RtkAudioManager(context, deviceStateChangeListener);
	}

	private RtkAudioManager(Context context, Runnable deviceStateChangeListener) {
		...
		// called.
		proximitySensor = RtkProximitySensor.create(context, new Runnable() {
...
//		RtkUtils.assertIsTrue(audioDevices.contains(device));
	...
	private void registerForWiredHeadsetIntentBroadcast() {
		...
		wiredHeadsetReceiver = new BroadcastReceiver() {
			...
			public void onReceive(Context context, Intent intent) {
				...
				Log.d(TAG,
						"BroadcastReceiver.onReceive" + RtkUtils.getThreadInfo() + ": " + "a=" + intent.getAction()
	...
	/**
	 * 更新的列表可能的音频设备,使新设备的选择。
	 * 
	 * @param hasWiredHeadset
	 *            是否有耳机
	 */
	...
	private void onAudioManagerChangedState() {
		...
		if (audioDevices.size() == 2) {
			RtkUtils.assertIsTrue(
					audioDevices.contains(AudioDevice.EARPIECE) && audioDevices.contains(AudioDevice.SPEAKER_PHONE));
